...
contract LiquidateNFT is Test {
    ...
    function setUp() public {
        ...
        vm.startPrank(alice);
        erc20.freemint();
        erc20.approve(address(strikePool), 10 ether);
        erc20.approve(address(auctionManager), 10 ether);
        vm.stopPrank();
        vm.startPrank(alice);
    ...
    function testLiquidateNFTBeforePeriod() public {
        vm.warp(2 * epochduration + interval - 1);
        vm.expectRevert(bytes("Liquidation period isn't reached"));
        strikePool.liquidateNFT(1);
    }
    ...
    function testLiquidateNFT() public {
        ...
        strikePool.liquidateNFT(1);
        vm.prank(alice);
        strikePool.bidAuction(1, 3 ether);
        vm.warp(3 * epochduration);
        strikePool.endAuction(1);
    }
}
